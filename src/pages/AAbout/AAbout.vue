...
<template>
  <VCard class="overflow-block a-about">
    <VCardSubtitle style="padding-top: 12px">🤔 Описание</VCardSubtitle>
    <VCardItem>
      Приложение позволяет группировать, фильтровать и сортировать группы, а так
      же быстро переходить к их фото\альбомам\видео\статьям. Встроенная галерея
      позволяет смотреть фото в максимальном качестве и листать фото нажатиями
      по краям, как в Telegram. То, что Вы просматриваете сохраняется в историю,
      но она обнуляется при каждом обновлении приложения, так как ВК меняет
      домен при публикации.
    </VCardItem>
